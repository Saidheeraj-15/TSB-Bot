...
# ── FOCUS MODE CONFIG ─────────────────────────────────────────────────────────
FOCUS_VC_ID      = 1507655441344364638
GRACE_PERIOD     = 60
...
class SetAlarmModal(discord.ui.Modal, title="Set Your Alarm"):
    time_input = discord.ui.TextInput(label="Time (IST) — 24hr e.g. 08:00", placeholder="HH:MM", max_length=5)
    message_input = discord.ui.TextInput(label="Reminder Message", placeholder="e.g. Solve today's LeetCode POTD!", max_length=200, style=discord.TextStyle.paragraph)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            return await interaction.response.send_message("❌ Invalid time. Use HH:MM e.g. `08:00`", ephemeral=True)
        try:
            ...
            return await interaction.response.send_message("❌ Failed to save alarm. Please try again.", ephemeral=True)
...
class AlarmDeleteSelect(discord.ui.Select):
    def __init__(self, user_alarms):
        ...
        super().__init__(placeholder="Select alarm(s) to delete...", min_values=1, max_values=len(opts), options=opts)
    async def callback(self, interaction: discord.Interaction):
        try:
            for aid in self.values: delete_alarm(int(aid))
        except Exception as ex:
            print(f"❌ delete_alarm error: {ex}")
            return await interaction.response.send_message("❌ Failed to delete. Please try again.", ephemeral=True)
        await interaction.response.send_message(f"🗑️ Deleted **{len(self.values)}** alarm(s).", ephemeral=True)
        self.view.stop()
...
class AlarmPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="Set Alarm", style=discord.ButtonStyle.primary, emoji="⏰", custom_id="alarm:set")
    async def set_alarm(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(embed=discord.Embed(title="⏰ New Alarm", description="Choose the day(s) first.", color=0x5865F2), view=DayPickerView(), ephemeral=True)
    @discord.ui.button(label="My Alarms", style=discord.ButtonStyle.secondary, emoji="📋", custom_id="alarm:list")
    async def my_alarms(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer(ephemeral=True)
        try:
            alarms = get_alarms(str(interaction.user.id))
        except Exception as ex:
            return await interaction.followup.send("❌ Failed to fetch alarms.", ephemeral=True)
        if not alarms: return await interaction.followup.send("📭 You have no alarms yet.", ephemeral=True)
        e = discord.Embed(title="📋 Your Alarms", color=0x5865F2)
        for a in alarms:
            e.add_field(name=f"#{a['id']}  •  {a['hour']:02d}:{a['minute']:02d} IST  •  {days_label(a['days'])}", value=f"💬 {a['message']}", inline=False)
        e.set_footer(text=f"{len(alarms)} active alarm(s)")
        await interaction.followup.send(embed=e, ephemeral=True)
    @discord.ui.button(label="Delete Alarm", style=discord.ButtonStyle.danger, emoji="🗑️", custom_id="alarm:delete")
    async def delete_alarms(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            return await interaction.followup.send("❌ Failed to fetch alarms.", ephemeral=True)
        if not alarms: return await interaction.followup.send("📭 No alarms to delete.", ephemeral=True)
        await interaction.followup.send(embed=discord.Embed(title="🗑️ Delete Alarms", color=0xED4245), view=AlarmDeleteView(alarms), ephemeral=True)

class ScheduleModal(discord.ui.Modal, title="One-Time Reminder"):
    date_input = discord.ui.TextInput(label="Date (IST) — DD/MM/YYYY", placeholder="e.g. 25/12/2025", max_length=10)
    time_input = discord.ui.TextInput(label="Time (IST) — HH:MM", placeholder="e.g. 08:00", max_length=5)
    message_input = discord.ui.TextInput(label="Reminder Message", placeholder="e.g. Submit the project!", max_length=200, style=discord.TextStyle.paragraph)
    async def on_submit(self, interaction: discord.Interaction):
        try:
            day, month, year = map(int, self.date_input.value.strip().split("/"))
        except:
            return await interaction.response.send_message("❌ Invalid date. Use DD/MM/YYYY e.g. `25/12/2025`", ephemeral=True)
        try:
            h, m = map(int, self.time_input.value.strip().split(":"))
            assert 0 <= h <= 23 and 0 <= m <= 59
        except:
        ...
        try:
            ...
            return await interaction.response.send_message("❌ That date doesn't exist.", ephemeral=True)
        if target <= datetime.now(IST):
            return await interaction.response.send_message("❌ That date/time is already in the past.", ephemeral=True)
        try:
            add_schedule(str(interaction.user.id), target.timestamp(), self.message_input.value.strip())
        except Exception as ex:
            return await interaction.response.send_message("❌ Failed to save reminder. Please try again.", ephemeral=True)
...
class ScheduleDeleteSelect(discord.ui.Select):
    def __init__(self, user_schedules):
        ...
        super().__init__(placeholder="Select reminder(s) to delete...", min_values=1, max_values=len(opts), options=opts)
    async def callback(self, interaction: discord.Interaction):
        try:
            for sid in self.values: delete_schedule(int(sid))
        except Exception as ex:
            return await interaction.response.send_message("❌ Failed to delete. Please try again.", ephemeral=True)
        await interaction.response.send_message(f"🗑️ Deleted **{len(self.values)}** reminder(s).", ephemeral=True)
        self.view.stop()
...
class SchedulePanelView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)
    @discord.ui.button(label="Schedule Reminder", style=discord.ButtonStyle.primary, emoji="📅", custom_id="schedule:set")
    ...
    @discord.ui.button(label="My Reminders", style=discord.ButtonStyle.secondary, emoji="📋", custom_id="schedule:list")
    async def list_schedules(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer(ephemeral=True)
        try:
            schedules = get_schedules(str(interaction.user.id))
        except Exception as ex:
            return await interaction.followup.send("❌ Failed to fetch reminders.", ephemeral=True)
        if not schedules: return await interaction.followup.send("📭 You have no upcoming reminders.", ephemeral=True)
        e = discord.Embed(title="📋 Your Reminders", color=0x5865F2)
        for s in schedules:
            dt = datetime.fromtimestamp(s["timestamp"], tz=IST)
            e.add_field(name=f"#{s['id']}  •  {dt.strftime('%d/%m/%Y %H:%M IST')}  •  ⏳ {time_until(dt)}", value=f"💬 {s['message']}", inline=False)
        e.set_footer(text=f"{len(schedules)} pending reminder(s)")
        await interaction.followup.send(embed=e, ephemeral=True)
    @discord.ui.button(label="Delete Reminder", style=discord.ButtonStyle.danger, emoji="🗑️", custom_id="schedule:delete")
    async def delete_schedule(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            return await interaction.followup.send("❌ Failed to fetch reminders.", ephemeral=True)
        if not schedules: return await interaction.followup.send("📭 No reminders to delete.", ephemeral=True)
        await interaction.followup.send(embed=discord.Embed(title="🗑️ Delete Reminders", color=0xED4245), view=ScheduleDeleteView(schedules), ephemeral=True)

class AdminAlarmDeleteSelect(discord.ui.Select):
    def __init__(self, alarms):
        opts = [discord.SelectOption(label=f"#{a['id']} [{a['_username']}] — {a['hour']:02d}:{a['minute']:02d} IST", description=a["message"][:50], value=str(a["id"])) for a in alarms[:25]]
        super().__init__(placeholder="Delete repeating alarm(s)...", min_values=1, max_values=len(opts), options=opts)
    async def callback(self, interaction: discord.Interaction):
        for aid in self.values: delete_alarm(int(aid))
        await interaction.response.send_message(f"🗑️ Deleted **{len(self.values)}** alarm(s).", ephemeral=True)
        self.view.stop()
...
class AdminScheduleDeleteSelect(discord.ui.Select):
    def __init__(self, schedules):
        opts = [discord.SelectOption(label=f"#{s['id']} [{s['_username']}] — {datetime.fromtimestamp(s['timestamp'],tz=IST).strftime('%d/%m/%Y %H:%M')}", description=s["message"][:50], value=str(s["id"])) for s in schedules[:25]]
        super().__init__(placeholder="Delete one-time reminder(s)...", min_values=1, max_values=len(opts), options=opts)
    async def callback(self, interaction: discord.Interaction):
        for sid in self.values: delete_schedule(int(sid))
        await interaction.response.send_message(f"🗑️ Deleted **{len(self.values)}** reminder(s).", ephemeral=True)
        self.view.stop()
...
async def spawn_panel(interaction: discord.Interaction):
    e = discord.Embed(title="⏰ Alarm Panel", color=0x5865F2, description="Get DM reminders on your chosen days at a set time.")
...
async def spawn_schedule(interaction: discord.Interaction):
    e = discord.Embed(title="📅 One-Time Reminder Panel", color=0x5865F2, description="Get a DM at a specific date & time. Fires once, then auto-removes.")
    e.set_footer(text="Responses are private • Times in IST")
...
async def admin_panel(interaction: discord.Interaction):
    ...
    if not all_alarms and not all_schedules:
        return await interaction.followup.send("📭 No data yet. Users haven't set any alarms or reminders.", ephemeral=True)
    uid_cache = {}
    ...
    if all_alarms:
        ...
        grouped = {}
        for a in all_alarms: grouped.setdefault(a["user_id"], []).append(a)
        annotated = []
    ...
    if all_schedules:
        ...
        grouped2 = {}
        for s in all_schedules: grouped2.setdefault(s["user_id"], []).append(s)
        annotated2 = []
...
async def daily_backup():
    ...
    try:
        ...
        e = discord.Embed(title="🗄️ Daily Backup", color=0x57F287, description=f"Auto-backup at {now}")
        ...
        for a in get_all_alarms():
            if a["hour"] == now_h and a["minute"] == now_m and now_day in a["days"]:
                try:
                    ...
                    e = discord.Embed(title="⏰ Alarm!", description=a["message"], color=0xFEE75C, timestamp=datetime.now(timezone.utc))
        ...
        for s in get_all_schedules():
            ...
            if target.date() == now_ist.date() and target.hour == now_h and target.minute == now_m:
                try:
                    ...
                    e = discord.Embed(title="📅 Reminder!", description=s["message"], color=0xEB459E, timestamp=datetime.now(timezone.utc))
...
def has_cam_or_screen(member: discord.Member) -> bool:
    if member.voice is None: return False
...
async def focus_session(member: discord.Member):
    e = discord.Embed(title="📚 Focus Mode Active",
        description="You joined the **study VC**!\n\n📷 Please turn on your **camera or screen share** within **1 minute** or you'll be removed.",
        color=0xFEE75C)
    ...
    if not has_cam_or_screen(member):
        e = discord.Embed(title="❌ Removed from Study VC", description="You didn't turn on your **camera or screen share** within 1 minute.\nRejoin when you're ready! 📚", color=0xED4245)
        await safe_dm(member, embed=e)
    ...
    while elapsed < FOCUS_DURATION:
        ...
        if member.voice is None or member.voice.channel.id != FOCUS_VC_ID:
            focus_tasks.pop(member.id, None); return
        if not has_cam_or_screen(member):
            e = discord.Embed(title="❌ Removed from Study VC", description="Your **camera and screen share** were both turned off during the session.\nRejoin when you're ready to focus again! 📚", color=0xED4245)
            await safe_dm(member, embed=e)
            await kick_from_vc(member)
            focus_tasks.pop(member.id, None); return
    e = discord.Embed(title="🎉 50 Minutes Done! Break Time.",
        description="Amazing work! You've completed a full **50 minute focus session**.\n\n😌 You have **10 minutes** to relax — camera/screen share can be off.\n⏰ You'll get a reminder **2 minutes before** your break ends.",
        color=0x5865F2)
    ...
    if member.voice is None or member.voice.channel.id != FOCUS_VC_ID:
        focus_tasks.pop(member.id, None); return
    e = discord.Embed(title="⚠️ Break Ending Soon!", description="You have **2 minutes left** in your break.\nPlease turn on your **camera or screen share** before time's up!", color=0xFEE75C)
    ...
    if member.voice is None or member.voice.channel.id != FOCUS_VC_ID:
        focus_tasks.pop(member.id, None); return
    if not has_cam_or_screen(member):
        e = discord.Embed(title="❌ Removed from Study VC", description="Your break ended and your **camera/screen share** was still off.\nRejoin to start a fresh session! 📚", color=0xED4245)
        await safe_dm(member, embed=e)
        await kick_from_vc(member)
        focus_tasks.pop(member.id, None); return
    e = discord.Embed(title="🔁 Starting New Session!", description="Back to focus! Another **50 minute session** begins now. Let's go! 💪", color=0x57F287)
    await safe_dm(member, embed=e)
...
if __name__ == "__main__":
    if not BOT_TOKEN:
        print("❌ DISCORD_BOT_TOKEN not set!"); exit(1)
    if not SUPABASE_URL or not SUPABASE_KEY:
        print("❌ SUPABASE_URL or SUPABASE_KEY not set!"); exit(1)
    print("🚀 Starting bot...")
